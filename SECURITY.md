...
This project treats **anything sent to the browser as public and hostile**: the client bundle,
...
enforce authorization. This file is the coverage map; the
...
| Input validation   | Validate at every trust boundary              | `[x]`  | Zod schemas at every boundary; Server Actions/Route Handlers parse input       |
...
| Auth / sessions    | Server-validated sessions; secure cookies     | `[ ]`  | Not applicable — the site has no accounts                                      |
| Authorization      | Deny-by-default; never trust the client       | `[ ]`  | Not applicable — no user-owned data is stored                                  |
| CSP (enforcing)    | Strict CSP with nonces                        | `[~]`  | Report-only baseline shipped; tighten and enforce before launch                |
...
- **Never trust the client for authorization** — the server decides.
- **No open redirects** — route user-supplied redirect targets through `safeRedirectPath`.
- **Review any diff touching env, Server Actions, headers, or redirects** against
  [`docs/security/checklist.md`](docs/security/checklist.md) before merge.
